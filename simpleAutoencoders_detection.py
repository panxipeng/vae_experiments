...
encoder = load_model('Compiled/simpleAutoencoders_encoder.h5')
...
# Save classifier
classifier.save('Compiled/Classifiers/simpleAutoencoders.h5')

# Save history
ae_history = np.array(classifier_history.history['loss'])
np.savetxt('history_classifier_sA.csv', ae_history, delimiter=',')

# Training Sets Precision
result_neg = classifier.predict(x_classi_negative)
result_pos = classifier.predict(x_classi_positive)
np.savetxt('Training_rawResult_sA.csv', np.concatenate((result_pos,result_neg), axis=0), delimiter=',')
...
np.savetxt('Training_Precision_sA.csv', np.array([np.count_nonzero(result_pos == 1), np.count_nonzero(result_neg == 0)]), delimiter=',')

# Test Sets Precision
result_neg = classifier.predict(test_classi_negative)
result_pos = classifier.predict(test_classi_positive)
np.savetxt('Test_rawResult_sA.csv', np.concatenate((result_pos,result_neg), axis=0), delimiter=',')
result_neg[result_neg<0.2] = 0
result_pos[result_pos>0.8] = 1
print('Test Sets Precision:')
print(np.count_nonzero(result_pos == 1) / 13400)
print(np.count_nonzero(result_neg == 0) / 13400)
np.savetxt('Test_Precision_sA.csv', np.array([np.count_nonzero(result_pos == 1), np.count_nonzero(result_neg == 0)]), delimiter=',')