% This script aims to extract negative samples from datasets, for trainning
% usage.
...
for i = 1:img_range
    ...
    img = imread(Img_File);
    load(Mat_File);
    total_nucleis = length(detection);
%     figure(1);
%     imshow(img);
%     hold on;
...
csvwrite('nucleis_neg_data_original.dat',nucleis_neg_data(2:end,:));
csvwrite('nucleis_neg_data.dat',nucleis_neg_data(2:10001,:));
% save workspace
save('train_neg_data_workspace')
